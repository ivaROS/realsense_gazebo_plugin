#include "realsense_gazebo_plugin/gazebo_ros_realsense_pt.h"
...
namespace gazebo {
// Register the plugin
GZ_REGISTER_MODEL_PLUGIN(GazeboRosRealsensePt)

GazeboRosRealsensePt::GazeboRosRealsensePt() {}

GazeboRosRealsensePt::~GazeboRosRealsensePt() {
  ROS_DEBUG_STREAM_NAMED("realsense_camera", "Unloaded");
}

void GazeboRosRealsensePt::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  RealSensePluginPt::Load(_model, _sdf);
...
void GazeboRosRealsensePt::OnNewFrame(const rendering::CameraPtr cam,
...
bool GazeboRosRealsensePt::FillPointCloudHelper(sensor_msgs::PointCloud2 &point_cloud_msg,
...
void GazeboRosRealsensePt::OnNewDepthFrame() {
  // get current time
  common::Time current_time = this->world->GetSimTime();

  RealSensePluginPt::OnNewDepthFrame();
