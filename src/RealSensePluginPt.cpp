...
#include "realsense_gazebo_plugin/RealSensePluginPt.h"
...
/////////////////////////////////////////////////
RealSensePluginPt::RealSensePluginPt() {
...
/////////////////////////////////////////////////
void RealSensePluginPt::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  // Output the name of the model
  std::cout
      << std::endl
      << "RealSensePluginPt: The realsense_camera plugin is attach to model "
  ...
  if (!this->depthCam) {
    std::cerr << "RealSensePluginPt: Depth Camera has not been found"
              << std::endl;
    return;
  }
  if (!this->ired1Cam) {
    std::cerr << "RealSensePluginPt: InfraRed Camera 1 has not been found"
              << std::endl;
    return;
  }
  if (!this->ired2Cam) {
    std::cerr << "RealSensePluginPt: InfraRed Camera 2 has not been found"
              << std::endl;
    return;
  }
  if (!this->colorCam) {
    std::cerr << "RealSensePluginPt: Color Camera has not been found"
              << std::endl;
    return;
  }
  ...
  try {
    ...
    std::cerr << "RealSensePluginPt: depthMap allocation failed: " << e.what()
              << std::endl;
    return;
  }
  ...
  this->newDepthFrameConn = this->depthCam->ConnectNewDepthFrame(
      std::bind(&RealSensePluginPt::OnNewDepthFrame, this));

  this->newIred1FrameConn = this->ired1Cam->ConnectNewImageFrame(std::bind(
      &RealSensePluginPt::OnNewFrame, this, this->ired1Cam, this->ired1Pub));

  this->newIred2FrameConn = this->ired2Cam->ConnectNewImageFrame(std::bind(
      &RealSensePluginPt::OnNewFrame, this, this->ired2Cam, this->ired2Pub));

  this->newColorFrameConn = this->colorCam->ConnectNewImageFrame(std::bind(
      &RealSensePluginPt::OnNewFrame, this, this->colorCam, this->colorPub));

  // Listen to the update event
  this->updateConnection = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&RealSensePluginPt::OnUpdate, this));
}

/////////////////////////////////////////////////
void RealSensePluginPt::OnNewFrame(const rendering::CameraPtr cam,
...
/////////////////////////////////////////////////
void RealSensePluginPt::OnNewDepthFrame() {
...
/////////////////////////////////////////////////
void RealSensePluginPt::OnUpdate() {}
